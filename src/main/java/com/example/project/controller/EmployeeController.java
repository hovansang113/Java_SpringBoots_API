...
public class EmployeeController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<EmployeeDTO>> save(@Valid @RequestBody EmployeeRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.created(employeeService.save(request)));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<EmployeeDTO>> updateById(@PathVariable Long id,
